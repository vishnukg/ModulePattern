...
| [09-production-build.md](./09-production-build.md)   | Native Node.js execution, optional tsup build, Three Musketeers (Make/Docker/Compose), CI |
...
| Dependency Inversion Principle | `DB` interface in `ports/db.ts`, implemented in `adapters/db/`                              |
...
| Three Musketeers               | `Makefile` + `docker-compose.yml` — `make ci` runs the same in dev and CI                   |
