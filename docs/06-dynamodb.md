...
- `cancelReservation` and `updateReservation` resolve "did it exist?" inside the
  write itself — `ReturnValues: "ALL_OLD"` on the delete, and
  `ConditionExpression: "attribute_exists(id)"` on the put (a failed condition
  throws `ConditionalCheckFailedException`, which the adapter maps to `null`).
  A get-then-write pair would cost two round trips and race against concurrent
  requests; the single conditional call is atomic.
- `getReservations` loops on `LastEvaluatedKey` because a `Scan` paginates at
  1 MB — without the loop, a big table would silently return only its first page.
