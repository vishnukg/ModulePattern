...
import { makeCancel } from "../src/core/index.ts";
...
describe("makeCancel — cancellation logic", () => {
  it("returns Cancelled when the reservation exists", async () => {
    const cancel = makeCancel({ db: stubDb, logger: stubLogger, metrics: stubMetrics });
  ...
  it("returns NotFound when the reservation does not exist", async () => {
    ...
    const cancel = makeCancel({ db, logger: stubLogger, metrics: stubMetrics });
...
describe("makeCancel — db interaction", () => {
  it("calls db.cancelReservation with the id", async () => {
    const mockDb: DB = { ...stubDb, cancelReservation: vi.fn(async () => true) };
    const cancel = makeCancel({ db: mockDb, logger: stubLogger, metrics: stubMetrics });
...
describe("makeCancel — logging", () => {
  it("calls logger.info on successful cancellation", async () => {
    ...
    const cancel = makeCancel({ db: stubDb, logger: mockLogger, metrics: stubMetrics });
  ...
  it("calls logger.warn when not found", async () => {
    const db: DB = { ...stubDb, cancelReservation: async () => false };
    const mockLogger = { info: vi.fn(), warn: vi.fn(), error: vi.fn() };
    const cancel = makeCancel({ db, logger: mockLogger, metrics: stubMetrics });
...
describe("makeCancel — metrics", () => {
  it("increments reservation.cancelled on success", async () => {
    ...
    const cancel = makeCancel({ db: stubDb, logger: stubLogger, metrics: mockMetrics });
  ...
  it("does not increment on NotFound", async () => {
    const db: DB = { ...stubDb, cancelReservation: async () => false };
    const mockMetrics = { increment: vi.fn(), timing: vi.fn() };
    const cancel = makeCancel({ db, logger: stubLogger, metrics: mockMetrics });
  ...
  it("records a timing for every attempt regardless of outcome", async () => {
    ...
    expect(mockMetrics.timing).toHaveBeenCalledWith("reservation.cancel_ms", expect.any(Number));
  });
});
