...
import makeUpdate from "../src/modules/restaurant/makeUpdate.ts";
...
describe("makeUpdate — update logic", () => {
  it("returns Updated when the reservation exists and quantity fits", async () => {
    const update = makeUpdate({ db: stubDb, restaurantCfg: { tableSize: 10 }, logger: stubLogger, metrics: stubMetrics });
  ...
  it("returns NotFound when the reservation does not exist", async () => {
    ...
    const update = makeUpdate({ db, restaurantCfg: { tableSize: 10 }, logger: stubLogger, metrics: stubMetrics });
...
describe("makeUpdate — db interaction", () => {
  it("calls db.updateReservation with the id and input", async () => {
    const mockDb: DB = { ...stubDb, updateReservation: vi.fn(async () => updatedReservation) };
    const update = makeUpdate({ db: mockDb, restaurantCfg: { tableSize: 10 }, logger: stubLogger, metrics: stubMetrics });

    await update("abc-123", { quantity: 4, date: "25/12/12" });

    expect(mockDb.updateReservation).toHaveBeenCalledWith("abc-123", { quantity: 4, date: "25/12/12" });
  });
...
describe("makeUpdate — logging", () => {
  it("calls logger.info on success", async () => {
    ...
    const update = makeUpdate({ db: stubDb, restaurantCfg: { tableSize: 10 }, logger: mockLogger, metrics: stubMetrics });
  ...
  it("calls logger.warn on NotFound", async () => {
    const db: DB = { ...stubDb, updateReservation: async () => null };
    const mockLogger = { info: vi.fn(), warn: vi.fn(), error: vi.fn() };
    const update = makeUpdate({ db, restaurantCfg: { tableSize: 10 }, logger: mockLogger, metrics: stubMetrics });
...
describe("makeUpdate — metrics", () => {
  it("increments reservation.update.accepted on success", async () => {
    ...
    const update = makeUpdate({ db: stubDb, restaurantCfg: { tableSize: 10 }, logger: stubLogger, metrics: mockMetrics });
  ...
  it("records a timing on success", async () => {
    ...
    expect(mockMetrics.timing).toHaveBeenCalledWith("reservation.update_ms", expect.any(Number));
  });
